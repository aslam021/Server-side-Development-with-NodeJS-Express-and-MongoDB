...
.get((req,res,next) => { 
    ...
    .catch((err) => next(err)); //go to the next
})
.post(authenticate.verifyUser, authenticate.verifyAdmin, (req, res, next) => {
    Dishes.create(req.body) //req.body. is obtained using bodyParser
    .then((dish) => {
...
.put(authenticate.verifyUser, authenticate.verifyAdmin, (req, res, next) => {
    Dishes.findByIdAndUpdate(
        req.params.dishId, 
        {$set: req.body},       //req.body. is obtained using bodyParser
        { new: true })
    .then((dish) => {
        res.statusCode = 200;
        res.setHeader('Content-Type', 'application/json');
...
.get((req,res,next) => {
    ...
    .then((dish) => {
        if (dish != null) {
            ...
            err.status = 404;
            return next(err); //this will be handled by error handlers in the app.js file
        }
    }, (err) => next(err))
    .catch((err) => next(err));
})
    ...
    .then((dish) => {
        if (dish != null && dish.comments.id(req.params.commentId) != null) {
            ...
            if(!dish.comments.id(req.params.commentId).author._id.equals(req.user._id)){
                var err = new Error("you cannot do this");
                err.status = 403;
                return next(err);
            }
